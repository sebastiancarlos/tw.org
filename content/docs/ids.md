...
But rebuilding is only run occasionally: it is run immediately before any command that displays ID numbers, such as a report, but not when a task is modified in any way.
...
This is why only commands that display ID numbers perform a rebuild first.
